...
public class NoticeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//uri는 .toString() 안붙여도 되는데 url은 붙여야됨.(http://localhost/jsp02/notice_servlet/* => 이런 형식으로 반환된다)
		...
		//----------- 페이징 처리 --------------//
			//notice 테이블의 레코드 갯수 계산 (dao의 count 메소드를 이용해서 값을 받아옴)
			int count=dao.count();
		...
		//----------- list.do 처리 --------------//
			...
			String page="/ui_page/communication/notice_list.jsp";
		...
		//공지사항 글 보기
			...
			//화면 전환
			String page="/ui_page/communication/notice_view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}//view()
		...
		//댓글 기능
			...
			request.setAttribute("list", list);
			//출력 페이지로 이동
			String page="/ui_page/communication/notice_comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}//commentList()
		...
		//댓글 삭제 기능
			...
			int notice_num=Integer.parseInt(request.getParameter("n_num"));
			String writer=request.getParameter("writer");
		...
		/*
		//댓글에 답글 달기 기능
		else if(url.indexOf("replyComment.do") != -1) {
			
		}//reply()
		*/
		
		//게시물 수정or삭제 값 전달을 위한 컨트롤
		else if(url.indexOf("edit.do") != -1) {
			//해당 게시물 번호 
			int num=Integer.parseInt(request.getParameter("num"));
			String page="/ui_page/communication/notice_edit.jsp";
		...
		//검색해서 찾기
			...
			String page="/ui_page/communication/notice_search.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		}
	}
